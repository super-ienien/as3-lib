...
package caspar.network
{
    ...
    public class ServerConnection extends EventDispatcher
	{
		 ...
		 * @return
		 */
		public function SendCommand(command:String, defer:Boolean = false):String 
		{
			_socket.addCommand( { command: defer ? command + ' DEFER':command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 */
		public function MixerKeyer(channel:uint, layer:uint, keyer:int, defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' KEYER ' + keyer + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 */
		public function MixerBlend(channel:uint, layer:uint, blend:String, defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' BLEND ' + blend + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 */
		public function MixerOpacity(channel:uint, layer:uint, opacity:Number, duration:uint = 0, tween:String = "linear", defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' OPACITY ' + opacity + ' ' + duration + ' ' + tween + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 * @param	tween The transition you want, see http://casparcg.com/wiki/CasparCG_Server_2.0b#Animation_Types
		 */
		public function MixerBrightness(channel:uint, layer:uint, brightness:Number, duration:uint = 0, tween:String = "linear", defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' BRIGHTNESS ' + brightness + ' ' + duration + ' ' + tween + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 * @param	tween The transition you want, see http://casparcg.com/wiki/CasparCG_Server_2.0b#Animation_Types
		 */
		public function MixerSaturation(channel:uint, layer:uint, saturation:Number, duration:uint = 0, tween:String = "linear", defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' SATURATION ' + saturation + ' ' + duration + ' ' + tween + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 * @param	tween The transition you want, see http://casparcg.com/wiki/CasparCG_Server_2.0b#Animation_Types
		 */
		public function MixerContrast(channel:uint, layer:uint, contrast:Number, duration:uint = 0, tween:String = "linear", defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' CONTRAST ' + contrast + ' ' + duration + ' ' + tween + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 */
		public function MixerLevels(channel:uint, layer:uint, min_input:Number, max_input:Number, gamma:Number, min_output:Number, max_output:Number, duration:uint = 0, tween:String = "linear", defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' LEVELS ' + min_input + ' ' + max_input + ' ' + gamma + ' ' + min_output + ' ' + max_output + ' ' + duration + ' ' + tween + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 * @param	tween The transition you want, see http://casparcg.com/wiki/CasparCG_Server_2.0b#Animation_Types
		 */
		public function MixerFill(channel:uint, layer:uint, x:Number, y:Number, scaleX:Number, scaleY:Number, duration:uint = 0, tween:String = "linear", defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' FILL ' + x + ' ' + y + ' ' + scaleX + ' ' + scaleY + ' ' + duration + ' ' + tween + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 * @param	tween The transition you want, see http://casparcg.com/wiki/CasparCG_Server_2.0b#Animation_Types
		 */
		public function MixerClip(channel:uint, layer:uint, x:Number, y:Number, scaleX:Number, scaleY:Number, duration:uint = 0, tween:String = "linear", defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' CLIP ' + x + ' ' + y + ' ' + scaleX + ' ' + scaleY + ' ' + duration + ' ' + tween + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 * @param	tween The transition you want, see http://casparcg.com/wiki/CasparCG_Server_2.0b#Animation_Types
		 */
		public function MixerCrop(channel:uint, layer:uint, left:Number, top:Number, right:Number, bottom:Number, duration:uint = 0, tween:String = "linear", defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' CROP ' + left + ' ' + top + ' ' + right + ' ' + bottom + ' ' + duration + ' ' + tween + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}		
		 ...
		 * @param	tween The transition you want, see http://casparcg.com/wiki/CasparCG_Server_2.0b#Animation_Types
		 */
		public function MixerGrid(channel:uint, resolution:uint, duration:uint = 0, tween:String = "linear", defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + ' GRID ' + resolution + ' ' + duration + ' ' + tween + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		 ...
		 * @param	tween The transition you want, see http://casparcg.com/wiki/CasparCG_Server_2.0b#Animation_Types
		 */
		public function MixerVolume(channel:uint, layer:uint, volume:Number, duration:uint = 0, tween:String = "linear", defer:Boolean = false):String 
		{
			var command:String = 'MIXER ' + channel + '-' + layer + ' VOLUME ' + volume + ' ' + duration + ' ' + tween + (defer ? ' DEFER':'') + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}
		...
		public function MixerCommit(channel:uint, layer:int = -1):String 
		{
			var command:String = 'MIXER ' + channel + (layer != -1 ? "-" + layer : "") + ' COMMIT' + SOCKET_COMMAND_END;
			_socket.addCommand( { command: command, type: ServerConnectionEvent.ON_OTHER_COMMAND } );
			return command;
		}		
...
class CustomSocket extends Socket
{
	...
	public var queueMode:String = 'salvo';
	
	private var _flushScheduled:Boolean;
    private var _response:String;
	private var _commandQueue:Array;
	private var _pendingCommandQueue:Array;
	...
	private function nextCommand():void 
	{
		if (queueMode == 'sequential')
		{
			...
			{
				_pendingCommandQueue.push(_commandQueue.splice(0,1)[0]);
				dispatchEvent(new ServerConnectionEvent(ServerConnectionEvent.ON_SEND_COMMAND, false, false, "", _pendingCommandQueue[0].command));
				sendRequest(_pendingCommandQueue[0].command);
			}			
		}
		else if (!_flushScheduled)
		{
			_flushScheduled = true;
			setTimeout(_flushQueue, 0);
		}

	}

	private function _flushQueue()
	{
		_flushScheduled = false;
		_response = "";
		_pendingCommandQueue = _commandQueue.splice(0);
		for (var i = 0, l = _pendingCommandQueue.length; i<l; i++)
		{
			dispatchEvent(new ServerConnectionEvent(ServerConnectionEvent.ON_SEND_COMMAND, false, false, "", _pendingCommandQueue[i].command));
			writeln(_pendingCommandQueue[i].command);
			writeln("\r\n");
			trace(_pendingCommandQueue[i].command);
		}
		flush();
	}
	...
	{
		_pendingCommandQueue.splice(0, 1);
		if (queueMode == 'sequential') nextCommand();
		//trace (_pendingCommandQueue.length);
	}
    ...
    private function readResponse():void
	{
		...
		if(_pendingCommandQueue[0].type == ServerConnectionEvent.ON_GET_TEMPLATES || _pendingCommandQueue[0].type == ServerConnectionEvent.ON_GET_MEDIAFILES)
		{
			...
			{
				dispatchEvents(_response, _pendingCommandQueue[0].command, _pendingCommandQueue[0].type);
